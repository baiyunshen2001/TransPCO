...
saveRDS(q2_matrix,"q2_matrix_TIE.rds")

q2_null_matrix=matrix(NA,nrow = n.sim, ncol=K)
for(N.sim in 1:n.sim){
  data=gene_data_null(n,p,A_snp_cis,A_cis_trans,A_trans,MAF_sample)
  data_ind=gene_data_null(700,p,A_snp_cis,A_cis_trans,A_trans,MAF_sample)
  Z_matrix_null=matrix(nrow = p,ncol = ncol(A_trans))
  for (i in 1:p) {
    for (j in 1:ncol(A_trans)) {
      res_z=susieR::univariate_regression(data$G[,i],data$E_trans[,j])
      Z_matrix_null[i,j]=res_z$betahat/res_z$sebetahat
    }
  }
  maf=data$maf
  SigmaGE_null=Z_matrix/sqrt(2*n*maf*(1-maf))
...
saveRDS(q2_null_matrix,"q2_null_matrix_TIE.rds")
...
saveRDS(q2_matrix,"q2_matrix_power.rds")
...
saveRDS(q2_null_matrix,"q2_null_matrix_power.rds")
...
saveRDS(pvalue_TIE,"pvalue_TIE.rds")
saveRDS(pvalue_power,"pvalue_power.rds")